...
def _model(modules, init, ranges):
    ...
    return SimpleNamespace(
        modules=modules, init=init, range_dict=ranges, next_str=NEXT)
...
def test_sound_pruning():
    ...
    # v to 3: impossible.
    m = _model(
        modules=[{"flow": _flow(v=1), "inv": And([Leq(Real("v"), RealVal("1"))]),
                  "jump": {}}],
...
def test_no_false_pruning():
    ...
    modules = [
        {"flow": _flow(v=1), "inv": And([Leq(Real("v"), RealVal("1"))]),
         "jump": {Geq(Real("v"), RealVal("0")): _identity_reset("v")}},
        {"flow": _flow(v=0), "inv": And([]), "jump": {}},
    ]
    m = _model(modules, And([Eq(Real("v"), RealVal("0"))]),
               {Real("v"): _rng(0, 10)})
    f = LinearWordFeasibilityFilter(m, time_bound=3)
...
def test_property_independence():
    ...
    assert all("prop" not in p and "stl" not in p and "goal" not in p
...
def test_nonlinear_conservative():
    ...
    m = _model(
        modules=[{"flow": Ode([Real("v")], [Mul(Real("v"), Real("v"))]),
                  "inv": And([Leq(Real("v"), RealVal("1"))]), "jump": {}}],
        init=And([Eq(Real("v"), RealVal("0"))]),
        ranges={Real("v"): _rng(0, 10)})
    f = LinearWordFeasibilityFilter(m, time_bound=3)
...
def test_guard_disjunction_sound():
    ...
    modules = [
        {"flow": _flow(v=1, w=0),
         ...
         "jump": {Geq(Real("v"), RealVal("2")): _identity_reset("v", "w"),
...
def _three_mode_climb():
    ...
    def climb(target):
        return {"flow": _flow(v=1), "inv": And([Leq(Real("v"), RealVal("2"))]),
                "jump": {Geq(Real("v"), RealVal("2")): And(
                    [Eq(Real("m" + NEXT), RealVal(str(target))),
                     Eq(Real("v" + NEXT), Real("v"))])}}
    modules = [climb(1), climb(2),
               {"flow": _flow(v=0), "inv": And([]), "jump": {}}]
    return _model(modules, And([Eq(Real("v"), RealVal("0"))]),
                  {Real("v"): _rng(0, 10)})
...
def test_time_horizon_prunes():
    ...
    m = _model([{"flow": _flow(v=1), "inv": And([Leq(Real("v"), RealVal("10"))]),
                 "jump": {}}],
               And([Eq(Real("v"), RealVal("0"))]), {Real("v"): _rng(0, 100)})
    assert LinearWordFeasibilityFilter(
        m, time_bound=5).word_is_infeasible(0, [0]) is False
    assert LinearWordFeasibilityFilter(
        m, time_bound=5, time_horizon=2).word_is_infeasible(0, [0]) is True
...
def test_exact_rate_is_not_pruned():
    ...
    # required.
    m = _model([{"flow": Ode([Real("v")], [Div(RealVal("5"), RealVal("3"))]),
                 "inv": And([Leq(Real("v"), RealVal("5"))]), "jump": {}}],
               And([Eq(Real("v"), RealVal("0"))]), {Real("v"): _rng(0, 100)})
    f = LinearWordFeasibilityFilter(m, time_bound=3)
    assert f.word_is_infeasible(0, [0]) is False