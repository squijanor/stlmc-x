...
``validate_gen`` runs at strategy start and rejects a malformed value there,
naming the key, the value and the expectation, before model parsing or any
solver work. The per-call bound additionally rejects a negative
``query-timeout`` at resolution time, since a negative bound leaves z3 unbounded
and passes a negative timeout to the dReal path's ``Queue.get``.
"""
...
class TestValidateGen:
    ...
    def test_an_unknown_key_is_not_this_check_s_business(self):
        """Unrecognised keys are read as absent; the run banner is what makes
        them visible. Validation must not reject them."""
