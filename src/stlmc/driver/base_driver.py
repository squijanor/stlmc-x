            ...
            for goal in goals:
                ...
                if final_result == "False":
                    ...
                    if gen_result == "true" or is_pool:
                        ...
                        # ".counterexample". A pool carries three further elements: per-CE labels
                        # on the tenth (the empty list where a strategy has none), the relaxation
                        # the backend answered under on the eleventh, and one structure-signature
                        # record per counterexample on the twelfth, so a pool says for itself
                        # whether its entries are witnesses or candidates and carries each entry's
                        # word and skeleton. All are written on the pool path only; the single-CE
                        # tuple is unchanged.
                        ...
                        if is_pool:
                            ...
                            from ..generation.encode import structure_signature_records

                            payload = payload + (
                                list(ce_labels) if ce_labels is not None else [],
                                float(backend_precision(config, underlying_solver)),
                                structure_signature_records(assn_dict),
                            )
